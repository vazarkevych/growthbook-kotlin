...
- **Java 17 is the runtime floor.** The module is built with a pinned JDK 17 toolchain, so the
  published bytecode is Java 17 (class file 61) regardless of the JDK that built it (#250). That
  matches the core SDK's JVM artifact, which this module depends on via `api` — a lower target
  would be a promise it could not keep. Neither Redis client forces it down: Jedis 8 and Lettuce 7
  are both Java 8 bytecode.
